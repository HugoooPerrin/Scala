...
object TimeUsage {
  ...
  def timeUsageByLifePeriod(): Unit = {
    val (columns, initDf) = read("/home/hugoperrin/Bureau/Datasets/Timeusage/atussum.csv")
  ...
  def dfSchema(columnNames: List[String]): StructType = columnNames match {
    case head :: tail => StructType(
      StructField(head, StringType, false) :: tail.map(StructField(_, DoubleType, false))
    )
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val work = columnNames.filter(name => name.startsWith("t05") || name.startsWith("t1805"))
                          .map(col(_))

    val primaryNeed = columnNames.filter(name => name.startsWith("t01") || name.startsWith("t03") || name.startsWith("t11") ||
                                                 name.startsWith("t1801") || name.startsWith("t1803"))
                                 .map(col(_))

    val leisure = columnNames.filter(name => (name.startsWith("t02") || name.startsWith("t04") || name.startsWith("t06") ||
                                              ...
                                              name.startsWith("t18")) &&
                                              !(name.startsWith("t1801") || name.startsWith("t1803") || name.startsWith("t1805")))
                             .map(col(_))

    (primaryNeed, work, leisure)
  }
  ...
  /** @return a projection of the initial DataFrame such that all columns containing hours spent on primary needs
    ...
    */
  def timeUsageSummary(primaryNeedsColumns: List[Column], workColumns: List[Column],
                       otherColumns: List[Column], df: DataFrame): DataFrame = {
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed.groupByKey((row: TimeUsageRow) => (row.working, row.sex, row.age)) // typed equivalent
